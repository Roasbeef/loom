...
# a commit-bound JSON manifest, and SHA256SUMS over the artifacts.
#
...
# as a dependency wants. The name identifies its build platform; the BEAM
# shipment itself still runs with compatible OTP on another supported host.
...
SLIM_STEM="loom-slim-$VERSION-$PLAT"
...
# One writer normalises metadata and binds every archive to the source commit.
python3 scripts/release-archive.py --root "$ROOT" --output "$DIST" \
  --platform "$PLAT" --version "$VERSION" --server "$REL" \
  --client "$CLIENT_REL" --slim "$SLIM_STAGE"
rm -rf "$SLIM_STAGE"
