...
# The commit the generated launcher exports as LOOM_BUILD_COMMIT. It is
# resolved here, at build time, because the tree the launcher runs in is a
...
case "$DEBUG" in
  0) DEBUG_INFO=strip ;;
  1) DEBUG_INFO=keep ;;
  *) echo "release.sh: DIST_DEBUG must be 0 or 1" >&2; exit 1 ;;
esac
...
# Normal releases strip BEAM debug information. DIST_DEBUG=1 keeps it and
# adds OTP tools for instrument, tprof and msacc in the diagnostic release.
#
...
APPS="client, runtime_tools"
[ "$CODEMODE" = 1 ] && APPS="$APPS, compiler"
...
# Every release carries the observational census. `runtime_tools` makes its
# allocator census available in the profiled VM even when its BEAM files were
# stripped. Loom already has an application named tools, so a debug release
# copies OTP's tool modules without tools.app rather than replacing Loom's
...
if [ "$DEBUG" = 1 ]; then
  ...
  DEBUG_START='set -- -pa "$root/share/diagnostics/ebin" "$@"
if [ -n "${LOOM_DEBUG_ARGS_FILE:-}" ]; then
  set -- -name loom_debug@127.0.0.1 -args_file "$LOOM_DEBUG_ARGS_FILE" \
    -kernel inet_dist_use_interface "{127,0,0,1}" "$@"
  unset LOOM_DEBUG_ARGS_FILE
fi'
fi
cat > "$REL/bin/loom-profile" <<EOF
#!/usr/bin/env bash
# Generated by scripts/release.sh. Attach the bundled census to a profile node.
set -euo pipefail
...
cat > "$REL/bin/loom-profile-config" <<EOF
#!/usr/bin/env bash
# Generated by scripts/release.sh. Reads daemon.profile before loomd's VM starts.
set -euo pipefail
here=\$(CDPATH= cd -- "\$(dirname -- "\$0")" && pwd -P)
root=\$(dirname "\$here")
config=\${1:?usage: loom-profile-config <catalogue>}

exec "\$root/erts-$ERTS_VSN/bin/erl" \\
  -boot "\$root/releases/$VERSION/no_dot_erlang" \\
  -pa "\$root"/lib/*/ebin \\
  -noshell \\
  -eval 'case init:get_plain_arguments() of [Config] -> case file:read_file(Config) of {ok, Text} -> case tom:parse(Text) of {ok, Document} -> case {tom:get_bool(Document, [<<"daemon">>, <<"profile">>]), '\''client@catalog'\'':parse(Text)} of {{ok, true}, {ok, _}} -> halt(0); _ -> halt(1) end; _ -> halt(1) end; _ -> halt(1) end; _ -> halt(2) end.' \\
  -extra "\$config"
EOF
chmod +x "$REL/bin/loom-profile-config"

# The launcher exports the build identity so the running VM can report which
# build it is (#392). The version is fixed by this release and the commit is
# resolved above; both are baked into the file rather than read at run time,
# because the release is unpacked on machines with no git and no checkout.
...
if (( LOOM_PROFILE_ENABLED == 1 )); then
  unset LOOM_DEBUG_ARGS_FILE
  set -- -name "\$LOOM_PROFILE_NODE" +Muatags true -env HOME "\$LOOM_PROFILE_ORIGINAL_HOME" \\
    -kernel inet_dist_use_interface '{127,0,0,1}' "\$@"
fi
$DEBUG_START
...
exec \${profile_exec[@]+"\${profile_exec[@]}"} "\$root/erts-$ERTS_VSN/bin/erl" \\
  -boot "\$root/releases/$VERSION/no_dot_erlang" \\
  -pa "\$root"/lib/*/ebin \\
  -noshell \\
