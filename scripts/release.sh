...
if [ "$SMOKE" = 0 ]; then
  ...
  [ "$DEBUG" = 1 ] && need erlc "compiles the bundled diagnostic probe"
...
[ "$DEBUG" = 1 ] && APPS="$APPS, runtime_tools"
...
# Loom already has an application named tools. Copy the OTP tools modules
# without its tools.app so loading profilers cannot replace Loom's metadata.
DEBUG_START=""
if [ "$DEBUG" = 1 ]; then
