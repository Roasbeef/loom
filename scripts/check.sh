...
packages=(host core storage session machine prompt telemetry runtime provider broker mcp lsp tools cap ext codemode events client tui conformance lint sandbox)
targets=("${@:-${packages[@]}}")
