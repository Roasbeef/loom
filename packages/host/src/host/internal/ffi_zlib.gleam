//// Bounded gzip inflation, the one external the archive
//// reader needs.
////
...
//// at a byte budget, so the shim in `host_zlib_ffi.erl` owns the loop that
...
//// lives here, backed by `host_zlib_ffi:inflate_gzip/2`.
...
/// ```
///
@external(erlang, "host_zlib_ffi", "inflate_gzip")
pub fn inflate_gzip(
  bytes: BitArray,
  limit: Int,
