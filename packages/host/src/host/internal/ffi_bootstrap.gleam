...
////
//// Startup policy belongs to callers. This module confines ambient OS calls
//// to the shared host_bootstrap_ffi.erl implementation.
...
// OTP monitors a port directly; no helper process or new Erlang shim is needed.
@external(erlang, "erlang", "monitor")
fn monitor_port(kind: PortMonitorFlag, lock: LaunchLock) -> Monitor

/// Monitors the original lock-helper port from its durable owner.
///
/// ## Examples
///
/// ```gleam
/// // ffi_bootstrap.lock_monitor(lock)
/// ```
...
/// Returns the current VM's operating-system PID through OTP os:getpid/0.
...
@external(erlang, "host_bootstrap_ffi", "system_time_ms")
...
@external(erlang, "host_bootstrap_ffi", "monotonic_time_ms")
...
@external(erlang, "host_bootstrap_ffi", "sha256")
...
@external(erlang, "host_bootstrap_ffi", "getenv")
...
@external(erlang, "host_bootstrap_ffi", "path_exists")
...
@external(erlang, "host_bootstrap_ffi", "absolute_path")
...
@external(erlang, "host_bootstrap_ffi", "ensure_private_directory")
...
@external(erlang, "host_bootstrap_ffi", "list_directory_bounded")
...
@external(erlang, "host_bootstrap_ffi", "read_regular_bounded")
...
@external(erlang, "host_bootstrap_ffi", "read_private_bounded")
...
/// mode bits.
@external(erlang, "host_bootstrap_ffi", "is_executable_file")
