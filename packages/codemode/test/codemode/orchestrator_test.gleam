...
fn ok_builder(
  ...
  Ok(compile.BuildProducts(beam_dir: root <> "/ebin", manifest_hash: "beef"))
}
...
pub fn vetting_rejection_short_circuits_test() {
  ...
  let assert codemode.VetRejected(rejections) = codemode.execute(source, config)
  assert rejections != []
}
