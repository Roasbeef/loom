//// The strand-name registry: one small actor mapping strand names to
//// the process names their drivers register under.
////
...
fn handle(
  state: Dict(String, Name(strand_runtime.Message)),
  message: Message,
) -> actor.Next(Dict(String, Name(strand_runtime.Message)), Message) {
  case message {
    Ensure(strand:, reply_with:) ->
      case dict.get(state, strand) {
        Ok(existing) -> {
        ...
        Error(Nil) -> {
          ...
          actor.continue(dict.insert(state, strand, fresh))
        }
      }
    Lookup(strand:, reply_with:) -> {
      process.send(reply_with, dict.get(state, strand))
      actor.continue(state)
    }
    Known(reply_with:) -> {
      process.send(reply_with, list.sort(dict.keys(state), string.compare))
      actor.continue(state)
    }
  }
}
