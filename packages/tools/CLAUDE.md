...
- **Shell scratch uses the published path.** `bash` directs the model to
  ...
  one command or background job, and both the description and the prompt
  snippet say so: a file a later call reads goes in `$TMPDIR`, the
  session's `<workspace>/.codemode/tmp`, which persists and which session
  assembly keeps out of `git status`. Pointing a model at "the workspace"
  instead sent its clones and notes into the repository it was editing.
  The helper retains explicit `TMPDIR` for compilers and the session
  fallback when no private scratch is available.
