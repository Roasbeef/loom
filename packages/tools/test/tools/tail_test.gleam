//// The bounded output tail: what a reader sees, what it is told it
...
//// the ones a reader of `client/jobs` or `tools/tool.collect_observed`
//// will rely on without re-deriving:
...
pub fn empty_tail_answers_with_nothing_test() {
  let tail = tail.new(capacity: 8)
  assert tail.received(tail) == 0
  assert tail.since(tail, 0) == tail.Since(bytes: <<>>, cursor: 0, dropped: 0)
}

pub fn a_chunk_inside_the_window_is_returned_whole_test() {
  let tail = tail.push(tail.new(capacity: 64), <<"hello":utf8>>)
  assert tail.since(tail, 0)
    == tail.Since(bytes: <<"hello":utf8>>, cursor: 5, dropped: 0)
}

pub fn a_cursor_at_the_end_sees_nothing_new_test() {
  let tail = tail.push(tail.new(capacity: 64), <<"hello":utf8>>)
  let seen = tail.since(tail, 0)
  assert tail.since(tail, seen.cursor)
    == tail.Since(bytes: <<>>, cursor: 5, dropped: 0)
}
...
pub fn successive_reads_concatenate_to_the_stream_test() {
  ...
  let #(_tail, cursor, seen) =
    list.fold(chunks, #(tail.new(capacity: 64), 0, <<>>), fn(carried, chunk) {
      let #(tail, cursor, seen) = carried
      let tail = tail.push(tail, chunk)
      let read = tail.since(tail, cursor)
...
pub fn a_flood_past_the_capacity_stays_bounded_test() {
  ...
  let tail =
    list.fold(list.repeat(Nil, 512), tail.new(capacity: 4), fn(tail, _step) {
      tail.push(tail, <<"0123456789abcdef0123456789abcdef":utf8>>)
    })
  assert tail.received(tail) == 16_384
  let read = tail.since(tail, 0)
...
pub fn a_cursor_older_than_the_window_reports_the_gap_test() {
  ...
  // still there.
  let tail = tail.push(tail.new(capacity: 4), <<"0123456789":utf8>>)
  assert tail.since(tail, 3)
    == tail.Since(bytes: <<"6789":utf8>>, cursor: 10, dropped: 3)
}

pub fn a_cursor_inside_the_window_reports_no_gap_test() {
  let tail = tail.push(tail.new(capacity: 4), <<"0123456789":utf8>>)
  assert tail.since(tail, 8)
    == tail.Since(bytes: <<"89":utf8>>, cursor: 10, dropped: 0)
}

pub fn a_cursor_outside_the_stream_is_clamped_test() {
  let tail = tail.push(tail.new(capacity: 64), <<"abc":utf8>>)

  // Ahead of the stream: nothing new, and the cursor comes back to the
  // end rather than staying in the future.
  assert tail.since(tail, 99) == tail.Since(bytes: <<>>, cursor: 3, dropped: 0)

  // Behind the beginning: the whole stream, and nothing was dropped
  // because nothing before byte zero ever existed.
  assert tail.since(tail, -5)
    == tail.Since(bytes: <<"abc":utf8>>, cursor: 3, dropped: 0)
}
...
pub fn a_split_character_is_held_back_until_it_completes_test() {
  ...
  let tail = tail.push(tail.new(capacity: 64), head)
  let first = tail.since(tail, 0)
  assert first == tail.Since(bytes: <<>>, cursor: 0, dropped: 0)

  let tail = tail.push(tail, rest)
  let second = tail.since(tail, first.cursor)
  assert second == tail.Since(bytes: whole, cursor: 2, dropped: 0)
}
...
pub fn the_window_front_lands_on_a_character_boundary_test() {
  ...
  let tail = tail.push(tail.new(capacity: 5), <<"£££":utf8>>)
  let read = tail.since(tail, 0)
...
pub fn output_that_is_not_utf8_still_advances_the_cursor_test() {
  ...
  let tail = tail.push(tail.new(capacity: 64), <<0xFF, 0xFE, 0xFD>>)
  assert tail.since(tail, 0)
    == tail.Since(bytes: <<0xFF, 0xFE, 0xFD>>, cursor: 3, dropped: 0)
}
...
pub fn a_capacity_below_one_byte_is_raised_test() {
  ...
  // anything at all.
  let tail = tail.push(tail.new(capacity: 0), <<"abc":utf8>>)
  let read = tail.since(tail, 0)
