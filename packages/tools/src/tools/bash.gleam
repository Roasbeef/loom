...
pub fn tool(jobs: Jobs) -> tool.Tool {
  tool.Tool(
    ...
    description: "Run a shell command in the sandboxed workspace. The "
      ...
      <> "exposes private scratch at a different path. `LOOM_SCRATCH_DIR` "
      <> "is deleted when this one command or background job ends, so a "
      <> "file a later call reads (a clone, a download, notes) goes in "
      <> "`$TMPDIR`, which persists for the session and stays out of the "
      <> "repository. A command still running when "
