...
- `tui/transcript_lines.AdvisorMessage` names the three advisor frames the transcript
  ...
  a durable message and `tui/transcript_lines.advisor_lines` renders it, collapsed or whole,
...
- **Advisor pending-nudge panel**: `tui/surfaces.sync_advisor_nudges` issues an
  ...
  three transitions — `tui/surfaces.advisor_nudges_action` names them `ReadNudges`:
