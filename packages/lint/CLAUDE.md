...
A pure analysis over `glance`'s AST, plus one `glexer` token scan used as
a fail-closed backstop for the one rule where a parser miss would be a
policy hole rather than a missed suggestion. It is a *reporting* tool,
not a gate: see **Staging** below before wiring anything to its exit
code.
...
- `lint.check(path, code, policy) -> List(Finding)` — the whole library
  API. Pure, total given `glance` returns, and the only entry point
  anything outside the package should need.
- `lint/finding.{Rule, Finding, id, name, parse, render, rules}` — the
  vocabulary. `Rule` is `Unparseable | EagerFallback | NestingDepth |
  CatchAll | PanicInSource | BoundedLength`, printed as `R0`..`R5`.
...
- **Depends on**: `glance` (>= 7, for an AST that carries a `Span` on
  every expression and pattern), `glexer` (the R4 backstop), `simplifile`
  (file discovery), `argv`. **No Loom package**, in either direction —
...
**Every rule ships at warning level and `make check` does not gate on
any of them.** That is deliberate, and it is the `scripts/doc_check.sh`
precedent (D2, `docs/design-notes/four-decisions.md`): a check earns the
error tier by producing a census that is stable, decidable, and argued —
