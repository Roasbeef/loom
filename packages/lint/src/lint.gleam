...
/// How much of a source is read looking for the generated marker. A header
/// worth believing is the first thing in the file, and a fixed prefix keeps
/// the question the same size on a five-thousand-line generated module as
/// on a short one — R5, this tool's own rule, applied to itself.
const header_prefix: Int = 400

/// Whether a generator wrote this source rather than a person.
///
/// Every rule here advises an edit, and the literate rules bind whoever
/// writes the source. For these files that is `scripts/gen-sql.sh`,
/// `scripts/embed-sql-schema.py` and `scripts/gen-prelude.py`, so a finding
/// in one asks for a change the next `make gen-sql` would discard — and
/// `make check` re-derives and compares them precisely so that nobody makes
/// it. That is noise which never clears, and R1's and R5's promotion cases
/// rest on a census where every line is actionable (issue #73,
/// housekeeping). `lint/cli` drops these before scanning; nothing in the
/// rules themselves knows about the exemption.
///
/// The marker is the `DO NOT EDIT.` header each of those generators emits,
/// and it counts only in the leading comment block: a hand-written module
/// that quotes the phrase in prose further down is still hand-written.
/// Five files in the tree carry it — `storage/sql`, `storage/sql_schema`,
/// `storage/session_schema`, `events/sql` and `tools/prelude`.
///
/// ## Examples
///
/// ```gleam
/// is_generated("//// Code generated by parrot. DO NOT EDIT.\n")
/// // -> True
/// ```
pub fn is_generated(code: String) -> Bool {
  code
  |> string.slice(at_index: 0, length: header_prefix)
  |> string.split("\n")
  |> list.any(marks_generated)
}

/// One header line claiming the file was generated. The comment prefix is
/// required so that a string literal or a line of embedded SQL carrying the
/// phrase cannot exempt the module it sits in.
fn marks_generated(line: String) -> Bool {
  string.starts_with(string.trim_start(line), "//")
  && string.contains(line, "DO NOT EDIT")
}
