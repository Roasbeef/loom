...
///
/// `cap/history`, `cap/memory` and `cap/context` are on this list
/// because `policy.extension_cap_modules` is the workspace seam widened,
/// and a capability added there reaches an extension by construction.
/// What an extension actually meets today is `unsupported_cap`: the
...
pub fn the_extension_allowlist_is_pinned_test() {
  let expected = [
    ...
    "cap/kv", "cap/lsp", "cap/memory", "cap/net", "cap/proc", "cap/report",
    "cap/schedule", "cap/search", "cap/task", "ext", "ext/hook", "ext/memory",
