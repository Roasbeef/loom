...
type StdinUse {
  ...
  // run and the question here is what *each* ask carried. The runner
  // writes one document with no trailing newline, so the stub adds
  // the line break itself; without it two asks land on one line and
...
fn stub(rig: Rig, name: String, stdin_use: StdinUse, tail: String) -> Nil {
  ...
  let stdin_line = case stdin_use {
    ...
    KeepStdin ->
      "{ cat; echo; } >> \"$CLAUDE_PROJECT_DIR/" <> name <> ".jsonl\"\n"
  }
  write(
    path,
    "#!/bin/sh\n"
