...
- `events/search.{Search, SearchError, Hit, open, sync, query,
  query_in_session, register_source, source, remove}` — the FTS5 index and its durable per-session
  ...
  session. `search_source` stores host-registered session paths for exact
