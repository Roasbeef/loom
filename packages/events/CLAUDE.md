...
- `events/search.{Search, SearchError, Hit, open, sync, query,
  query_in_session, recent_in_session, register_source, source, remove}` — the FTS5 index and its durable per-session
  ...
  session. `recent_in_session` needs no query: it answers one session's
  highest rowids, which FTS5 assigns in insertion order, so they are its
  newest entries, newest first. `search_source` stores host-registered session paths for exact
  reads; consumers validate the source identity before returning content.
  Removing a session drops its text, cursor and source locator together.
...
- `storage/sqlite_policy` supplies the shared database tuning. Search uses its
  defaults and can override typed fields without duplicating PRAGMA strings.
  Open failures close the acquired connection before returning an error.
