...
PACKAGES := core storage session machine prompt telemetry runtime provider \
	broker mcp tools cap codemode events client conformance tui_gleam lint
...
.PHONY: binaries
binaries: sandbox tui-shipment ## Build bin/loom-exec and the native bin/loom-tui launcher
	@install -m 0755 $(HELPER) bin/loom-exec
	@echo "built bin/loom-exec and bin/loom-tui"

.PHONY: tui-shipment
tui-shipment: ## Package the native TUI: build/tui-erlang-shipment + bin/loom-tui
	@cd packages/tui_gleam && gleam export erlang-shipment
	@mkdir -p bin build
	@rm -rf build/tui-erlang-shipment
	@cp -R packages/tui_gleam/build/erlang-shipment build/tui-erlang-shipment
	@printf '%s\n' '#!/bin/sh' \
		'set -eu' \
		...
		'exec erl +Bd -pa "$$SHIPMENT"/*/ebin -eval '\''tui_gleam@@main:run(tui_gleam)'\'' -noshell -extra "$$@"' \
		> bin/loom-tui
	@chmod +x bin/loom-tui
	@echo "built build/tui-erlang-shipment and bin/loom-tui"

.PHONY: server-shipment
server-shipment: ## Package the server: build/erlang-shipment + bin/loom-server (needs erl to run)
...
run-tui: binaries ## Attach the TUI: make run-tui ADDR=ws://host:port/v1/ws SESSION=id [TOKEN_FILE=path]
	...
	@if [ -n "$(TOKEN_FILE)" ]; then \
		./bin/loom-tui --addr "$(ADDR)" --session "$(SESSION)" \
			--token-file "$(TOKEN_FILE)"; \
	else ./bin/loom-tui --addr "$(ADDR)" --session "$(SESSION)"; fi
