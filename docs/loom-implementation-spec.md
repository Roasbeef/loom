...
- `hello` carries `{proto, peer: "exec-helper"|"satellite"|..., features: [..]}`; version mismatch → close, naming both versions. `proto` is the exec channel's *body* vocabulary version and is **3**; it is not the envelope's `"v"`, which describes the container above and stays 1. A protocol change that adds, removes, or makes-required a key on a frame the exec helper sends or receives — or adds a kind to that channel — bumps `proto` on both sides in the same commit; the addendum to [protocol-change/006](../protocol-change/006-exec-exit-cancelled.md) is the ruling and carries the mapping from each value to the change that earned it.
