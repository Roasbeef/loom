...
- **Stanzas**: a comment must have a blank line above it — between statements, between `case` arms, and between the variants of a custom type — and a body should not run more than about eight statements without a break. Counted in statements, never lines: a wrapped literal is one statement and a `use` chain is a table. **Lint R10 gates**; R11 warns. Its two exemptions are the two places `gleam format` deletes a blank line (the top of a block, and between a constructor's fields), because a rule must never demand what the formatter removes. gleam-style Part II, "Stanzas: how code breathes". This is the layout half of the literate comment register: prose a reader cannot find is prose that was not written.
...
- `hello` carries `{proto, peer: "exec-helper"|"satellite"|..., features: [..]}`; version mismatch → close, naming both versions. `proto` is the exec channel's *body* vocabulary version and is **3**; it is not the envelope's `"v"`, which describes the container above and stays 1. A protocol change that adds, removes, or makes-required a key on a frame the exec helper sends or receives — or adds a kind to that channel — bumps `proto` on both sides in the same commit; [protocol-change/017](../protocol-change/017-exec-protocol-version.md) is the ruling, and the addendum to [protocol-change/006](../protocol-change/006-exec-exit-cancelled.md) carries the full argument and the mapping from each value to the change that earned it.
...
[Protocol 050](../protocol-change/050-reasoning-summaries.md) adds summaries of long blocks. The daemon's `summarize` role writes a one- or two-sentence summary of each reasoning block and each delivered advice or nudges message whose text is at least 512 bytes, and stores it in the reserved `fact.custom` cell `summary/<entry_id>/<block_index>`; a reasoning block is sent only to a summarizer served from the same endpoint (scheme and host of the catalogue entry's `base_url`, or its dialect when it has none) as the entry that produced it. The pushed `block_summary{subject, …, text}` event carries a stored summary (`subject: "block"`, with `entry` and `block`) or an unstored summary of a reasoning stream still being written (`subject: "stream"`, with `strand`, `op` and the stream's `generation`). The subscribed, read-only `block_summaries{blocks}` reads up to 32 stored summaries by exact key and answers a `snapshot` of mode `block_summaries`. A client presents a summary as the summarizer's text, never as the agent's.
