...
`wrong_session`. Source: (`client/gateway.gleam:1782-1685`).
...
Body is identical to `steer`. Source: (`client/protocol.gleam:921`).
...
1. `expected_seq` MUST equal the record's current sequence. A mismatch
   is `stale_approval`. Source: (`client/gateway.gleam:5293-5279`).
...
8. **Two operation phases are missing from the documented label set.**
   ...
   `checkpoint` (`client/gateway.gleam:3114`) and `navigating`
