...
`wrong_session`. Source: (`client/gateway.gleam:1901`).
...
Body is identical to `steer`. Source: (`client/protocol.gleam:1108`).
...
1. `expected_seq` MUST equal the record's current sequence. A mismatch
   is `stale_approval`. Source: (`client/gateway.gleam:5755`).
...
8. **Two operation phases are missing from the documented label set.**
   ...
   `checkpoint` (`client/gateway.gleam:3328`) and `navigating`
