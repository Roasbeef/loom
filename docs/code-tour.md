...
`handle_text` becomes `dispatch` (`client/gateway.gleam:3230`), which
...
The hint carries nothing. It triggers `pull` (`client/gateway.gleam:2395`),
...
`reply_with_matched` (`client/gateway.gleam:4257`) pulls, picks the last
...
`Ctx.grants` (`run_grants`, `client/wiring.gleam:1198`). That is the
...
registry (`run_tool`, `client/wiring.gleam:1007`). All four come from the driver, so a
...
tool only when
`codemode.discover` (`client/codemode.gleam:881`) finds `gleam` and `erl`
...
`code_change` (`client_ffi.erl:286`). **Distribution is not used
