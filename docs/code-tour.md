...
`handle_text` becomes `dispatch` (`client/gateway.gleam:2991`), which
...
The hint carries nothing. It triggers `pull` (`client/gateway.gleam:2206`),
...
`reply_with_matched` (`client/gateway.gleam:3960`) pulls, picks the last
