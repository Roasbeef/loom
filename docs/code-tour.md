...
`spawn_helper` (`broker/exec.gleam:1879`) is where the Erlang side meets
...
`api.create_strand` (`runtime/api.gleam:816`) then seeds the child's
...
`await_strand_result` (`runtime/api.gleam:1043`) keys on the *operation*,
