...
`handle_text` becomes `dispatch` (`client/gateway.gleam:3661`), which
...
The hint carries nothing. It triggers `pull` (`client/gateway.gleam:2794`),
...
`reply_with_matched` (`client/gateway.gleam:5432`) pulls, picks the last
