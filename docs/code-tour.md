...
`handle_text` becomes `dispatch` (`client/gateway.gleam:3133`), which
...
The hint carries nothing. It triggers `pull` (`client/gateway.gleam:2301`),
...
`reply_with_matched` (`client/gateway.gleam:4127`) pulls, picks the last
