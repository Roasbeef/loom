...
`handle_text` becomes `dispatch` (`client/gateway.gleam:2645`), which
...
The hint carries nothing. It triggers `pull` (`client/gateway.gleam:1948`),
...
`reply_with_matched` (`client/gateway.gleam:3353`) pulls, picks the last
