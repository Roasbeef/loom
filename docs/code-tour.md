...
`handle_text` becomes `dispatch` (`client/gateway.gleam:3337`), which
...
The hint carries nothing. It triggers `pull` (`client/gateway.gleam:2532`),
...
`reply_with_matched` (`client/gateway.gleam:4864`) pulls, picks the last
