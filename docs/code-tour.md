...
`handle_text` becomes `dispatch` (`client/gateway.gleam:3246`), which
...
The hint carries nothing. It triggers `pull` (`client/gateway.gleam:2411`),
...
`reply_with_matched` (`client/gateway.gleam:4273`) pulls, picks the last
