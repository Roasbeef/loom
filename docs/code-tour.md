...
`handle_text` becomes `dispatch` (`client/gateway.gleam:3297`), which
...
The hint carries nothing. It triggers `pull` (`client/gateway.gleam:2461`),
...
`reply_with_matched` (`client/gateway.gleam:4324`) pulls, picks the last
