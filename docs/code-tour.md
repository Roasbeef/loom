...
`handle_text` becomes `dispatch` (`client/gateway.gleam:3001`), which
...
The hint carries nothing. It triggers `pull` (`client/gateway.gleam:2216`),
...
`reply_with_matched` (`client/gateway.gleam:3995`) pulls, picks the last
