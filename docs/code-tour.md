...
`handle_text` becomes `dispatch` (`client/gateway.gleam:3151`), which
...
The hint carries nothing. It triggers `pull` (`client/gateway.gleam:2317`),
...
`reply_with_matched` (`client/gateway.gleam:4145`) pulls, picks the last
