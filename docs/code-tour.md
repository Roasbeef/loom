...
`handle_text` becomes `dispatch` (`client/gateway.gleam:2762`), which
...
The hint carries nothing. It triggers `pull` (`client/gateway.gleam:1998`),
...
`reply_with_matched` (`client/gateway.gleam:3483`) pulls, picks the last
