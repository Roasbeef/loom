...
`handle_text` becomes `dispatch` (`client/gateway.gleam:3460`), which
...
The hint carries nothing. It triggers `pull` (`client/gateway.gleam:2624`),
...
`reply_with_matched` (`client/gateway.gleam:5176`) pulls, picks the last
