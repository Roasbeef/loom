...
`handle_text` becomes `dispatch` (`client/gateway.gleam:2889`), which
...
The hint carries nothing. It triggers `pull` (`client/gateway.gleam:2105`),
...
`reply_with_matched` (`client/gateway.gleam:3619`) pulls, picks the last
