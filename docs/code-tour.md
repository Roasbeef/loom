...
`handle_text` becomes `dispatch` (`client/gateway.gleam:3596`), which
...
The hint carries nothing. It triggers `pull` (`client/gateway.gleam:2751`),
...
`reply_with_matched` (`client/gateway.gleam:5299`) pulls, picks the last
...
For `bash`, `call_spec` (`tools/bash.gleam:663`) builds a `CallSpec` naming the
