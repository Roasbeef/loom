...
`handle_text` becomes `dispatch` (`client/gateway.gleam:3240`), which
...
The hint carries nothing. It triggers `pull` (`client/gateway.gleam:2405`),
...
`reply_with_matched` (`client/gateway.gleam:4267`) pulls, picks the last
