...
`handle_text` becomes `dispatch` (`client/gateway.gleam:3395`), which
...
The hint carries nothing. It triggers `pull` (`client/gateway.gleam:2589`),
...
`reply_with_matched` (`client/gateway.gleam:5018`) pulls, picks the last
...
`Ctx.grants` (`run_grants`, `client/wiring.gleam:1620`). That is the
...
registry (`run_tool`, `client/wiring.gleam:1539`). All four come from the driver, so a
