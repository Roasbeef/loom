...
(`extension_registrations` at `client/serve.gleam:1476`). A `Refused`
...
per session (`extension_hosts.supervised` at `client/serve.gleam:2527`)
...
| `codemode/satellite.gleam` | Both shapes of node: `run` for one execution, and the persistent `Host` (`codemode/satellite.gleam:1907`) with `start`, `invoke` (`codemode/satellite.gleam:2103`) and `stop`. |
...
| `client/serve.gleam` | The boot that finds what is installed: `extension_registrations` (`client/serve.gleam:1476`), the two refusals it logs, and the contribution it appends. |
...
| `tui/tui.gleam` | `loom ext …` forwarded to the server by the same ladder a local session uses; the `Forward` arm is at `tui.gleam:504`. |
