...
(`extension_registrations` at `client/serve.gleam:1467`). A `Refused`
...
(`framing.HookCall` at `broker/exec.gleam:1737`), because the two
...
| `client/serve.gleam` | The boot that finds what is installed: `extension_registrations` (`client/serve.gleam:1467`), the two refusals it logs, and the contribution it appends. |
