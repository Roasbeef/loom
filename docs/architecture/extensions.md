...
(`extension_registrations` at `client/serve.gleam:1436`). A `Refused`
...
per session (`extension_hosts.supervised` at `client/serve.gleam:2311`)
...
| `client/serve.gleam` | The boot that finds what is installed: `extension_registrations` (`client/serve.gleam:1436`), the two refusals it logs, and the contribution it appends. |
