...
(`extension_registrations` at `client/serve.gleam:2028`). A `Refused`
...
per session (`extension_hosts.supervised` at `client/serve.gleam:3387`)
...
| `client/serve.gleam` | The boot that finds what is installed: `extension_registrations` (`client/serve.gleam:2028`), the two refusals it logs, and the contribution it appends. |
...
| `tui/tui.gleam` | `loom ext …` forwarded to the server by the same ladder a local session uses; the `Forward` arm is at `tui.gleam:828`. |
