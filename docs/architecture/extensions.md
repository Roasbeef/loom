...
(`extension_registrations` at `client/serve.gleam:1500`). A `Refused`
...
per session (`extension_hosts.supervised` at `client/serve.gleam:2432`)
...
| `client/serve.gleam` | The boot that finds what is installed: `extension_registrations` (`client/serve.gleam:1495`), the two refusals it logs, and the contribution it appends. |
...
| `tui/tui.gleam` | `loom ext …` forwarded to the server by the same ladder a local session uses; the `Forward` arm is at `tui.gleam:409`. |
