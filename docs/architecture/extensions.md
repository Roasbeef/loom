...
1. **At boot, discovery feeds the registry.** `serve.assemble` reads
   ...
   registry (`extension_registrations` at `client/serve.gleam:2050`). A
...
per session (`extension_hosts.supervised` at `client/serve.gleam:3433`).
...
| `client/serve.gleam` | The boot that finds what is installed: `extension_registrations` (`client/serve.gleam:2050`), the two refusals it logs, and the contribution it appends. |
