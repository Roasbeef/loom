...
(`tui.gleam:329`). Two ladders would mean installing an extension into
...
(`extension_registrations` at `client/serve.gleam:2019`). A `Refused`
...
per session (`extension_hosts.supervised` at `client/serve.gleam:3346`)
...
produced the last of those. `invoke` (`extension/hosts.gleam:354`) is
what a tool call goes through; `invoke_event`
(`extension/hosts.gleam:446`) is the function the hook bus calls, and
...
(`contributions.registry` at `client/contributions.gleam:189`). An
...
| `codemode/vet/policy.gleam` | The four seams — the fourth, `resident` (`vet/policy.gleam:459`), is frozen for a tier that does not exist. `extension_cap_modules` (`vet/policy.gleam:635`) and `extension_stdlib_modules` (`vet/policy.gleam:663`) are the widening, written as the workspace list widened so the superset is a fact about the code. |
...
| `client/serve.gleam` | The boot that finds what is installed: `extension_registrations` (`client/serve.gleam:2019`), the two refusals it logs, and the contribution it appends. |
| `client/contributions.gleam` | The tool registry as an ordered list of contributions: `registry` (`client/contributions.gleam:189`) and the collision that refuses a boot. |
...
| `tui/tui.gleam` | `loom ext …` forwarded to the server by the same ladder a local session uses; the `Forward` arm is at `tui.gleam:776`. |
