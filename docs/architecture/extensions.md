...
per session (`extension_hosts.supervised` at `client/serve.gleam:3026`)
...
| `tui/tui.gleam` | `loom ext …` forwarded to the server by the same ladder a local session uses; the `Forward` arm is at `tui.gleam:735`. |
