...
(`extension_registrations` at `client/serve.gleam:1605`). A `Refused`
...
per session (`extension_hosts.supervised` at `client/serve.gleam:2756`)
...
| `client/serve.gleam` | The boot that finds what is installed: `extension_registrations` (`client/serve.gleam:1605`), the two refusals it logs, and the contribution it appends. |
...
| `tui/tui.gleam` | `loom ext …` forwarded to the server by the same ladder a local session uses; the `Forward` arm is at `tui.gleam:577`. |
