...
(`extension_registrations` at `client/serve.gleam:1789`). A `Refused`
...
per session (`extension_hosts.supervised` at `client/serve.gleam:2821`)
...
| `client/serve.gleam` | The boot that finds what is installed: `extension_registrations` (`client/serve.gleam:1789`), the two refusals it logs, and the contribution it appends. |
