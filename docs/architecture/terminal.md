...
`update` (`tui.gleam:4780`) is three steps:
...
is where socket traffic enters the model. `update_tick` (`tui.gleam:4996`)
...
`reconcile_cut` and then `render_cut` (`tui.gleam:6558`). `Streamed` and
