...
`tap_provider(tap_preview_provider(...))` (`client/serve.gleam:2725`), so
...
(`apply_pushed`, `tui/session_channel.gleam:501`).
...
reads as `Noticed` (`tui/session_channel.gleam:96`) before deciding what
