...
binary. It starts one `commit_forwarder` (`client/gateway.gleam:1057`) per
session and subscribes the writer to it, so the gateway learns of each
commit. It also nests the two provider taps,
`tap_provider(tap_preview_provider(...))` (`client/serve.gleam:3039`), so
...
through `deliver` (`client/gateway.gleam:3021`). Both paths call
`check_binding` (`client/gateway.gleam:2130`) immediately before the
...
prompts per strand (`held_per_strand`, `client/gateway.gleam:748`). A
...
`drain_idle_strands` (`client/gateway.gleam:4667`) is called from
`pull_and_broadcast` (`client/gateway.gleam:2404`) after `state.live` has
