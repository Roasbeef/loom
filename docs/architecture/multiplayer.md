...
binary. It starts one `commit_forwarder` (`client/gateway.gleam:1047`) per
session and subscribes the writer to it, so the gateway learns of each
commit. It also nests the two provider taps,
`tap_provider(tap_preview_provider(...))` (`client/serve.gleam:3009`), so
...
through `deliver` (`client/gateway.gleam:3011`). Both paths call
`check_binding` (`client/gateway.gleam:2120`) immediately before the
...
prompts per strand (`held_per_strand`, `client/gateway.gleam:738`). A
...
`drain_idle_strands` (`client/gateway.gleam:4616`) is called from
`pull_and_broadcast` (`client/gateway.gleam:2394`) after `state.live` has
...
`AllHeld`; `drain_strand` (`client/gateway.gleam:4620`) then passes its complete
...
`send_queued` (`tui/session_channel.gleam:1119`) starts the catch-up at the
