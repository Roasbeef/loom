...
binary. It starts one `commit_forwarder` (`client/gateway.gleam:870`) per
...
through `deliver` (`client/gateway.gleam:2720`). Both paths call
`check_binding` (`client/gateway.gleam:1839`) immediately before the
...
prompts per strand (`held_per_strand`, `client/gateway.gleam:572`). A
...
`drain_idle_strands` (`client/gateway.gleam:3692`) is called from
`pull_and_broadcast` (`client/gateway.gleam:2104`) after `state.live` has
...
`send_queued` (`tui/session_channel.gleam:950`) starts the catch-up at the
...
reads as `Noticed` (`tui/session_channel.gleam:82`) before deciding what
