...
`tap_provider(tap_preview_provider(...))` (`client/serve.gleam:2647`), so
...
through `deliver` (`client/gateway.gleam:2661`). Both paths call
`check_binding` (`client/gateway.gleam:1741`) immediately before the
...
prompts per strand (`held_per_strand`, `client/gateway.gleam:532`). A
...
`send_queued` (`tui/session_channel.gleam:943`) starts the catch-up at the
