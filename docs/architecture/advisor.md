...
`ensure_strand` (`client/advisor.gleam:3107`) creates the advisor through
...
produce it. `judge` (`client/advisor.gleam:1479`) refuses any caller
...
The actor's `decide` (`client/advisor.gleam:1531`) writes the guard to
