...
ledger (`strand_names`, `client/gateway.gleam:2582`). The advisor has
...
`advisor_payload` (`tui.gleam:7364`) extracts one of three
`AdvisorMessage` variants and `advisor_lines` (`tui.gleam:7455`) renders
