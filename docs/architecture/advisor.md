...
`create_idle_strand` (`runtime/api.gleam:1301`) instead, which is the
...
ledger (`strand_names`, `client/gateway.gleam:2733`). The advisor has
...
`advisor_payload` (`tui.gleam:8842`) extracts one of five
`AdvisorMessage` variants and `advisor_lines` (`tui.gleam:8947`) renders
