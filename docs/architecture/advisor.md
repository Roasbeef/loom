...
`create_idle_strand` (`runtime/api.gleam:1471`) instead, which is the
...
`advisor_payload` (`tui.gleam:8947`) extracts one of five
`AdvisorMessage` variants and `advisor_lines` (`tui.gleam:9052`) renders
