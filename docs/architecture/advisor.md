...
ledger (`strand_names`, `client/gateway.gleam:2525`). The advisor has
...
`advisor_payload` (`tui.gleam:6892`) extracts one of three
`AdvisorMessage` variants and `advisor_lines` (`tui.gleam:6892`) renders
