...
The `advisor` route is a sixth routable role, parsed to `advisor_role` (`client/catalog.gleam:336`). It is `model.Custom("advisor")` rather than
...
`parse_advisor` (`client/catalog.gleam:1574`) reads the `[advisor]`
