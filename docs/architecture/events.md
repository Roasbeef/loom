...
`recent_in_session(session, limit)` takes no query at all. It answers one
...
With no `MATCH` there is nothing for `snippet()` to anchor on, and each
hit's excerpt is the first 160 characters of its indexed text. This is
what `history_search` answers when a model asks for its own session's
history without a query.
...
`query_in_session`, `recent_in_session` — takes a `core/ids.SessionId`, with no
...
to `[1, 50]`, and never sends an empty query: in the session scope it is
a browse through `recent_in_session`, and in the repository scope it is
refused in band with a worded message.
...
`src/events/sql/search.sql` holds twelve named queries, `scripts/gen-sql.sh`
