...
beside `extension_hosts` (`client/serve.gleam:2809`), bound to a
...
derived from a strand's one open operation, which is what a `LiveOp` names (`client/gateway.gleam:3806`),
