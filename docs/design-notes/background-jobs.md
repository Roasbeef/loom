...
the call returns the literal `[command timed out]` (`tools/bash.gleam:748`)
...
beside `extension_hosts` (`client/serve.gleam:3413`), bound to a
...
ledger with `max_outstanding: 1` (`bash.gleam:694`). So if a job cleared
