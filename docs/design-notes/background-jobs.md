...
beside `extension_hosts` (`client/serve.gleam:3352`), bound to a
...
derived from a strand's one open operation, which is what a `LiveOp` names (`client/gateway.gleam:4267`),
