...
`reserved_fact_key` (`runtime/api.gleam:2228`), one row in the table
...
beside `extension_hosts` (`client/serve.gleam:3383`), bound to a
...
derived from a strand's one open operation, which is what a `LiveOp` names (`client/gateway.gleam:4535`),
