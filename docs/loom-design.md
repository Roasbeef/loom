...
**Defaults**: workspace-write, network **off**, `.git/`, `~/.ssh`, credential paths protected. Secrets are injected into provider calls by the ProviderGateway from the OS keychain — never in tool environments, never in transcripts; executor environments are allowlist-constructed, not inherited. `Proxy(allowlist)` egress runs through a harness-owned proxy; the sandbox blocks direct sockets, making the allowlist enforceable and loggable. *(That sidecar is unbuilt. Until it exists the broker narrows `Proxy` to `Off` and reports the narrowing, so proxy mode fails closed and nothing ever claims an allowlist was enforced — spec Part 5, track 10.)*
...
**Approval escalation, Codex-style**: a sandbox denial (or pre-declared need) produces an in-band structured escalation → surfaced with the exact policy diff ("wants: network to registry.npmjs.org"), the tool that asked and a bounded preview of the arguments it asked with → on approval, *one* re-execution under the widened policy, recorded durably. The grant binds to that **action**, by a digest over the call's effective arguments: a call that would do something else can neither claim the record nor spend it, so consent is at the granularity of the command rather than of the appetite. "Approve similar for this session" widens the session policy explicitly, never silently.
